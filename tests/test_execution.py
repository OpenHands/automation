...
class TestUploadUsesQueryParams:
    ...
    async def test_upload_preserves_path_in_query(
        self, destination, sdk_http_transport
    ):
...
class TestPrivateEnvironmentInjection:
    ...
    async def test_execute_command_omits_env_values(self, mock_upload, mock_start_bash):
        ...
        assert env_upload.args[3] == f"/tmp/automation-{run_id}.tar.gz.env"
        assert secret.encode() in env_upload.args[2]

        command = mock_start_bash.await_args.args[2]
    ...
    async def test_blocking_command_omits_session_key(
        ...
        assert result.success is True
        assert mock_upload.await_count == 2
        assert session_key.encode() in mock_upload.await_args_list[1].args[2]
        command = mock_bash.await_args.args[2]
...
class TestPerRunTarballPath:
    ...
    async def test_bytes_upload_uses_per_run_path(self, mock_start_bash, mock_upload):
        ...
        uploaded_dest = mock_upload.call_args.args[3]  # (url, key, data, dest)
    ...
    async def test_url_download_uses_per_run_path(
        ...
        download_dest = mock_download_in_sandbox.call_args.args[3]
    ...
    async def test_bash_cmd_uses_per_run_path_and_cleans_up(
        ...
        bash_cmd = mock_start_bash.call_args.args[2]  # (url, key, command)
    ...
    async def test_no_run_id_falls_back_to_shared_constant(
        ...
        uploaded_dest = mock_upload.call_args.args[3]
        assert uploaded_dest == TARBALL_PATH
        bash_cmd = mock_start_bash.call_args.args[2]
    ...
    async def test_concurrent_runs_use_distinct_paths(
        ...
        upload_dests = {c.args[3] for c in mock_upload.call_args_list}
...
@pytest.mark.asyncio
async def test_sdk_upload_failure_never_starts_entrypoint(sdk_http_transport):
    requests = []

    def respond(request):
        requests.append(request)
        return httpx.Response(403, json={"detail": "Upload denied"})

    sdk_http_transport(respond)
    result = await execute_in_context(
        agent_url="http://server",
        session_key="key",
        entrypoint="python main.py",
        tarball_source=b"bundle",
        work_dir="/workspace",
    )
    assert result.success is False
    assert result.error and "403" in result.error
    assert all(request.url.path == "/api/file/upload" for request in requests)


@pytest.mark.asyncio
async def test_sdk_command_failure_never_starts_entrypoint(sdk_http_transport):
    commands = []

    def respond(request):
        if request.method == "POST":
            commands.append(request.content)
            return httpx.Response(200, json={"id": "download-command"})
    ...
    result = await execute_in_context(
        ...
        session_key="key",
        entrypoint="python main.py",
        tarball_source="https://bundles.example/automation.tar.gz",
        work_dir="/workspace",
    )
    assert result.success is False
    assert result.error and "connection failed" in result.error
    assert len(commands) == 1
    assert b"curl" in commands[0]
    assert b"python main.py" not in commands[0]
