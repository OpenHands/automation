...
class TestGetLastBashCommandResult:
    ...
    async def test_handles_http_errors(self, status, sdk_http_transport):
        import httpx

        sdk_http_transport(lambda _: httpx.Response(status))
        result = await get_last_bash_command_result("http://localhost:3000", "test-key")
        assert result.found is False
    ...
    async def test_command_result_states(self, items, expected, sdk_http_transport):
        import httpx

        sdk_http_transport(lambda _: httpx.Response(200, json={"items": items}))
        result = await get_last_bash_command_result("http://localhost:3000", "test-key")
        assert result == expected

    @pytest.mark.asyncio
    @pytest.mark.parametrize("command_id", [None, "abc123"])
    async def test_correlates_the_selected_command(
        self, command_id, sdk_http_transport
    ):
        import httpx
        ...
        await get_last_bash_command_result(
            "http://localhost:3000", "test-key", command_id=command_id
        )
