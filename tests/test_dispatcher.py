        ...
        *,
        ...
        ctx = ExecutionContext(
            ...
            runtime_conversation_id=(
                uuid.UUID(runtime_conversation_id) if runtime_conversation_id else None
            ),
        )
        backend.get_execution_context = AsyncMock(return_value=ctx)
    ...
    async def _dispatch(
        ...
        *,
        ...
        runtime_id = (
            uuid.UUID(
                conversation_id_for(
                    org_id, automation_id, trigger["source"], subject_key
                )
            )
            if subject_key
            else None
        ...
        backend.get_execution_context = AsyncMock(return_value=ctx)
        backend.build_env_vars = MagicMock(
            return_value={"AUTOMATION_CONVERSATION_ID": str(runtime_id)}
            if runtime_id
            else {}
        )
        backend.get_work_dir = MagicMock(return_value="/workspace")
        ...
        if runtime_id:
            async with async_session_factory() as session:
                updated = await session.get(AutomationRun, run_id)
                assert updated.conversation_id == str(runtime_id)
