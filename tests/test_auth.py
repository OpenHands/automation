...
class TestAuthentication:
    ...
    async def test_authenticate_extracts_llm_profile_metadata(
        self, mock_request, mock_http_client
    ):
        """Auth stores available and active LLM profile names when present."""
        mock_request.headers.get.return_value = "Bearer valid-api-key"
        users_me = {
            **MOCK_USERS_ME_RESPONSE,
            "llm_profiles": {
        ...
        assert result.llm_profile_names == frozenset(
            {"fast-profile", "careful-profile"}
        )
        assert result.active_llm_profile_name == "fast-profile"
