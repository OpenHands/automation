    ...
    select_query = (
        ...
        .where(
            ...
            AutomationRun.automation.has(
                and_(Automation.enabled.is_(True), Automation.deleted_at.is_(None))
            ),
        )
...
async def _execute_run(
    ...
    async def _fail(error: str, failure_kind: FailureKind | str | None = None) -> None:
        """Mark a failed run and apply the automation health policy."""
        ...
        await capture_automation_event(
            ...
            properties={
                ...
                "automation_disabled": not run.automation.enabled,
            },
        )
    ...
    try:
        ...
        await _fail(str(exc), FailureKind.CONFIG)
        return
        ...
        await backend.release_context(client, ctx)
        await _fail(str(exc), classify_exception(exc).kind)
        return
    ...
    try:
        ...
        await backend.release_context(client, ctx)
        await _fail(str(exc) or "Internal error", classify_exception(exc).kind)
        return
