...
from openhands.automation.health import AutomationFailure
...
async def apply_terminal_health(
    ...
    *,
    ...
    """Persist terminal metadata and update the automation health streak."""
    ...
    run.failure_kind = failure.kind.value if failure_kind is not None else None
    ...
    if automation is None:
        return
    unhealthy = (
        status == AutomationRunStatus.FAILED and failure.counts_toward_disablement
    )
    automation.consecutive_unhealthy_runs = (
        automation.consecutive_unhealthy_runs + 1 if unhealthy else 0
    )
    threshold = threshold or get_config().service.unhealthy_failure_threshold
    if unhealthy and automation.consecutive_unhealthy_runs >= threshold:
        automation.enabled = False
        automation.disabled_reason = error or failure.kind.value
        await session.execute(
            ...
            .values(status=AutomationRunStatus.SKIPPED, completed_at=utcnow())
        )
    await session.flush()
    ...
    try:
        async with session_factory() as session:
            ...
            if db_run and db_run.status == AutomationRunStatus.RUNNING:
                await mark_run_status(session, db_run, status, error_detail=error)
