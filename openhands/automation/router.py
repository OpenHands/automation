...
async def update_automation(
    ...
    if not was_enabled and update_data.get("enabled") is True:
        auto.consecutive_unhealthy_runs = 0
...
async def dispatch_automation(
    ...
    if not auto.enabled:
        raise HTTPException(
            ...
            detail=(
                f"Automation is disabled: {auto.disabled_reason or 'disabled by user'}"
            ),
        )
...
async def complete_run(
    ...
    if body.status == "FAILED":
        if isinstance(body.error, ConversationErrorEvent):
            values["error_detail"] = body.error.detail
            classification = body.error.classification
            if classification is not None:
                values["failure_kind"] = classification.kind.value
        elif body.error is not None:
    ...
    run.error_detail = (
        body.error.detail
        if (body.status == "FAILED" and isinstance(body.error, ConversationErrorEvent))
        else (body.error if body.status == "FAILED" else None)
    )
